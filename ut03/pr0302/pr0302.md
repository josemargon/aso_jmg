...
echo "5) Centímetros a Metros"
echo "6) Metros a Centímetros"

read opcion

case $opcion in
    1)
        echo "Introduce la cantidad en metros:"
        read metros
        km=$(echo "$metros / 1000" | bc -l)
        echo "$metros metros equivalen a $km kilómetros."
        ;;
    2)
        echo "Introduce la cantidad en kilómetros:"
        read kilometros
        metros=$(echo "$kilometros * 1000" | bc -l)
        echo "$kilometros kilómetros equivalen a $metros metros."
        ;;
    3)
        echo "Introduce la cantidad en pies:"
        read pies
        metros=$(echo "$pies * 0.3048" | bc -l)
        echo "$pies pies equivalen a $metros metros."
        ;;
    4)
        echo "Introduce la cantidad en metros:"
        read metros
        pies=$(echo "$metros * 3.28084" | bc -l)
        echo "$metros metros equivalen a $pies pies."
        ;;
    5)
        echo "Introduce la cantidad en centímetros:"
        read centimetros
        metros=$(echo "$centimetros / 100" | bc -l)
        echo "$centimetros centímetros equivalen a $metros metros."
        ;;
    6)
        echo "Introduce la cantidad en metros:"
        read metros
        centimetros=$(echo "$metros * 100" | bc -l)
        echo "$metros metros equivalen a $centimetros centímetros."
        ;;
    *)
...
case $opcion in
    1)
        ...
        if [ $? -eq 0 ]; then
            echo "El servicio $servicio ha sido iniciado correctamente."
        else
            echo "Hubo un error al intentar iniciar el servicio $servicio."
        fi
        ;;
    2)
        sudo systemctl stop $servicio
        if [ $? -eq 0 ]; then
            echo "El servicio $servicio ha sido detenido correctamente."
        else
            echo "Hubo un error al intentar detener el servicio $servicio."
        fi
        ;;
    3)
        sudo systemctl restart $servicio
        if [ $? -eq 0 ]; then
            echo "El servicio $servicio ha sido reiniciado correctamente."
        else
