...
$objetos = foreach ($linea in $lineas){
    ...
    $tiempoLimpio = $partes[1].Replace('_',"").Replace("/","-")
    $TimeStamp = (Get-Date $fechaLimpia).ToString("yyyy-MM-dd HH:mm")

    $Severity = $partes[0].split("]")[0].Replace("[","").trim()

    $User = $partes[2].split(":")[1].trim()
    if ($User -eq "ROOT"){
        $User = "administrador"
    }

    $eventID = $partes[0].split("ID:")[1].trim()

    $Action = $partes[3].split(":")[1].trim()
...
$objetos | Export-Csv -Path "reporte_seguridad.csv"
